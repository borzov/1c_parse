...
def _prepare_final_cp_details(cp_data: Dict, org_data_cache: Dict, report_name: str) -> Dict:
    ...
    Использует кэш для результатов нормализации.
    """
    ...
    cache_key = (current_inn, tuple(sorted(raw_names_set))) # Ключ кэша: ИНН + сортированный набор сырых имен
        ...
        if final_name_norm is None or final_name_norm == '?':
            ...
            if fallback_raw and fallback_raw != best_raw_name:
                 ...
                 if fb_norm and fb_norm != '?':
                     ...
                     final_legal_form = fb_form # Используем форму от fallback имени
                 else:
                     final_name_norm = best_raw_name if best_raw_name else "?" # Последний fallback - лучшее сырое
                     final_legal_form = detect_final_legal_form(final_name_norm, current_inn) # Переопределяем форму
            else:
                 final_name_norm = best_raw_name if best_raw_name else "?" # Последний fallback
                 final_legal_form = detect_final_legal_form(final_name_norm, current_inn) # Переопределяем форму
    ...
    if name_to_format and name_to_format != '?':
         if final_legal_form in ('ИП', 'ФЛ'):
             ...
             display_name = name_to_format # Для ЮЛ это само чистое имя
    details['display_name'] = display_name # Может быть None, если имя "?"
...
def generate_counterparty_annual_report_v5(transactions: List[Dict], detected_orgs_map: Dict[str, Dict], output_path: str):
    ...
    for agg_key, data in cp_aggregated_data.items():
        ...
        for year, y_data in sorted(data['years'].items()):
            ...
            y_entry = {'year': year, 'year_income': y_data['totals']['income'], 'year_expense': y_data['totals']['expense'], 'orgs': []}
...
def generate_org_comparison_report_v5(transactions: List[Dict], detected_orgs_map: Dict[str, Dict], output_path: str):
    """Генерирует отчет о взаимодействии контрагентов с организациями."""
    report_id = "Comparison_v5"
    if not jinja_env:
